...
#Minimum scripts to create infrastructure

#NOTE: Please change this as you see fir for your project!!!!!!!!! Don't leave as is.
region="us-west-2"
...
# Create All Subnets (if you need a DB, please create a second private subnet and make the variable be called private_rds_subnet above)
...
aws ec2 describe-route-tables --route-table-id $pub_route_id  --profile ${profile} --region ${region} #make sure ur shit attached and is active
...
aws ec2 describe-route-tables --route-table-id $priv_route_id  --profile ${profile} --region ${region} #make sure ur shit attached and is active
